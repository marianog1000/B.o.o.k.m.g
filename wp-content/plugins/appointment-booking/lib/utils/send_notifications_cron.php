    ...
    private function getCustomerAppointments( Entities\Notification $notification, DataHolders\Notification\Settings $settings )
    {
        ...
        if ( $settings->getAtHour() !== null ) {
            ...
            if ( $this->hours <= ( $settings->getAtHour() + $this->processing_interval ) ) {
                $query = sprintf(
                    'SELECT `ca`.* FROM `%s` `ca` LEFT JOIN `%s` `a` ON `a`.`id` = `ca`.`appointment_id`
                    WHERE DATE(`a`.`start_date`) BETWEEN DATE("%s") AND DATE("%s")',
                    Entities\CustomerAppointment::getTableName(),
                    Entities\Appointment::getTableName(),
                    date( 'Y-m-d', $this->timestamp - ( $settings->getOffsetHours() + $this->processing_interval ) * HOUR_IN_SECONDS ),
                    date( 'Y-m-d', $this->timestamp - $settings->getOffsetHours() * HOUR_IN_SECONDS )
                );
            } else {
                return $ca_list;
            }
            ...
            $query = sprintf(
                'SELECT `ca`.* FROM `%s` `ca` LEFT JOIN `%s` `a` ON `a`.`id` = `ca`.`appointment_id`
                WHERE `a`.`start_date` BETWEEN FROM_UNIXTIME(%d) AND FROM_UNIXTIME(%d)',
                Entities\CustomerAppointment::getTableName(),
                Entities\Appointment::getTableName(),
                $this->timestamp - ( abs( $settings->getOffsetHours() ) + $this->processing_interval ) * HOUR_IN_SECONDS,
                $this->timestamp - abs( $settings->getOffsetHours() ) * HOUR_IN_SECONDS
            );
        }
    ...
    private function getCustomerAppointmentsStatusChanged( Entities\Notification $notification, DataHolders\Notification\Settings $settings )
    {
        ...
        if ( $settings->getAtHour() !== null ) {
            ...
            if ( $this->hours <= ( $settings->getAtHour() + $this->processing_interval ) ) {
                $query = sprintf(
                    'SELECT `ca`.* FROM `%s` `ca` LEFT JOIN `%s` `a` ON `a`.`id` = `ca`.`appointment_id`
                    WHERE DATE(`ca`.`status_changed_at`) BETWEEN DATE("%s") AND DATE("%s")',
                    Entities\CustomerAppointment::getTableName(),
                    Entities\Appointment::getTableName(),
                    date( 'Y-m-d', $this->timestamp - ( $settings->getOffsetHours() + $this->processing_interval ) * HOUR_IN_SECONDS ),
                    date( 'Y-m-d', $this->timestamp - $settings->getOffsetHours() * HOUR_IN_SECONDS )
            ...
            $query = sprintf(
                'SELECT `ca`.* FROM `%s` `ca` LEFT JOIN `%s` `a` ON `a`.`id` = `ca`.`appointment_id`
                WHERE `ca`.`status_changed_at` BETWEEN FROM_UNIXTIME(%d) AND FROM_UNIXTIME(%d)',
                Entities\CustomerAppointment::getTableName(),
                Entities\Appointment::getTableName(),
                $this->timestamp - ( abs( $settings->getOffsetHours() ) + $this->processing_interval ) * HOUR_IN_SECONDS,
                $this->timestamp - abs ( $settings->getOffsetHours() ) * HOUR_IN_SECONDS
    ...
    private function getNewCustomerAppointments( Entities\Notification $notification, DataHolders\Notification\Settings $settings )
    {
        ...
        if ( $settings->getAtHour() !== null ) {
            ...
            if ( $this->hours <= ( $settings->getAtHour() + $this->processing_interval ) ) {
                $query = sprintf(
                    'SELECT `ca`.* FROM `%s` `ca` LEFT JOIN `%s` `a` ON `a`.`id` = `ca`.`appointment_id`
                    WHERE DATE(`ca`.`created`) BETWEEN DATE("%s") AND DATE("%s")',
                    Entities\CustomerAppointment::getTableName(),
                    Entities\Appointment::getTableName(),
                    date( 'Y-m-d', $this->timestamp - ( $settings->getOffsetHours() + $this->processing_interval ) * HOUR_IN_SECONDS ),
                    date( 'Y-m-d', $this->timestamp - $settings->getOffsetHours() * HOUR_IN_SECONDS )
            ...
            $query = sprintf(
                'SELECT `ca`.* FROM `%s` `ca` LEFT JOIN `%s` `a` ON `a`.`id` = `ca`.`appointment_id`
                WHERE `ca`.`created` BETWEEN FROM_UNIXTIME(%d) AND FROM_UNIXTIME(%d)',
                Entities\CustomerAppointment::getTableName(),
                Entities\Appointment::getTableName(),
                $this->timestamp - ( abs( $settings->getOffsetHours() ) + $this->processing_interval ) * HOUR_IN_SECONDS,
                $this->timestamp - abs( $settings->getOffsetHours() ) * HOUR_IN_SECONDS
    ...
    private function getLastCustomerAppointments( Entities\Notification $notification, DataHolders\Notification\Settings $settings )
    {
        ...
        if ( $settings->getAtHour() !== null ) {
            ...
            if ( $this->hours <= ( $settings->getAtHour() + $this->processing_interval ) ) {
                ...
                $replace['{sent_time_interval}'] = sprintf(
                    '"%s" BETWEEN DATE(DATE_ADD(`start_date`, INTERVAL %d HOUR)) AND DATE(DATE_ADD(`start_date`, INTERVAL %d HOUR))',
                    $this->mysql_today,
                    $settings->getOffsetHours(),
                    $settings->getOffsetHours() + $this->processing_interval
                );
            } else {
                return $ca_list;
            }
            ...
            $replace['{sent_time_interval}'] = sprintf(
                '"%s" BETWEEN DATE_ADD(`start_date`, INTERVAL %d HOUR) AND DATE_ADD(`start_date`, INTERVAL %d HOUR)',
                $this->mysql_now,
                abs( $settings->getOffsetHours() ),
                abs( $settings->getOffsetHours() ) + $this->processing_interval
            );
        }
        ...
        if ( $this->hours >= $settings->getSendAtHour() ) {
            $query = sprintf(
                ...
                WHERE `ca`.`id` IN(
                    ...
                    AND NOT EXISTS (
                        SELECT * FROM `{ab_sent_notifications}` `sn`
                        WHERE DATE(`sn`.`created`) >= DATE("%s")
                            AND `sn`.`notification_id` = %d
                            AND `sn`.`ref_id` = `ca`.`id`
                    )',
    ...
    private function getCustomersWithBirthday( Entities\Notification $notification, DataHolders\Notification\Settings $settings )
    {
        ...
        if ( $this->hours >= $settings->getSendAtHour() ) {
            $rows = (array) $wpdb->get_results( sprintf(
                'SELECT `c`.* FROM `%s` `c`
                WHERE `c`.`birthday` IS NOT NULL
                    AND DATE_FORMAT(DATE_ADD(`c`.`birthday`, INTERVAL %d HOUR), "%%m-%%d") = DATE_FORMAT("%s", "%%m-%%d")
                    AND NOT EXISTS (
                        SELECT * FROM `%s` `sn`
                        WHERE DATE(`sn`.`created`) = DATE("%s")
                            AND `sn`.`notification_id` = %d
    ...
    private function sendStaffAgenda( Entities\Notification $notification, DataHolders\Notification\Settings $settings )
    {
        if ( $this->hours >= $settings->getSendAtHour() ) {
            ...
            $rows = $wpdb->get_results( sprintf(
                ...
                WHERE `ca`.`status` IN("%s","%s")
                    AND DATE(DATE_ADD("%s", INTERVAL %d HOUR)) = DATE(`a`.`start_date`)
                    AND NOT EXISTS (
                        SELECT * FROM `%s` `sn` 
                        WHERE DATE(`sn`.`created`) = DATE("%s")
                            AND `sn`.`notification_id` = %d
